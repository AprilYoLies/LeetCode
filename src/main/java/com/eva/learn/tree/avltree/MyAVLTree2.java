...
public class MyAVLTree2<T extends Comparable<T>> {
	...
	class AVLTreeNode<E extends Comparable<E>> {
		...
		public AVLTreeNode(E key, AVLTreeNode<E> left, AVLTreeNode<E> right, AVLTreeNode<E> father) {
			this.key = key;
			this.left = left;
			this.right = right;
			this.father = father;
	...
	private AVLTreeNode<T> insert(AVLTreeNode<T> tree, T key, AVLTreeNode<T> father) {
		if (tree == null) {
			return new AVLTreeNode<>(key, null, null, father);
		}

		int cmp = key.compareTo(tree.key);
		if (cmp < 0) {
			tree.left = insert(tree.left, key, tree);
			...
			tree.right = insert(tree.right, key, tree);
